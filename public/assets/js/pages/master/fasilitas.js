$(function() {
    ...
    // INIT DATATABLE
    // ===================================
    let tbl = $('#tblFasilitas').DataTable({
        ajax: {
            url: BASE_URL + "master-data/fasilitas/list",
            dataSrc: 'data'
        },
        columns: [{
                data: null,
            ...
            },
            {
                ...
                render: function(id) {
                    return `
                <button class="btn btn-sm btn-warning btn-edit mb-2" data-id="${id}" title="Edit Fasilitas">
                    <i class="bi bi-pencil-square"></i>
                </button>
                <button class="btn btn-sm btn-danger btn-delete mb-2" data-id="${id}" title="Delete Fasilitas">
    ...
    $('#modalAdd').on('shown.bs.modal', function() {
        ...
        // ============================================
        // 1. SELECT2 KODE Fasilitas (AUTO UPPERCASE)
        // ============================================
        initSelect2(
            '#kodeFasilitas',
            "master-data/fasilitas/search/kode",
            "Masukan Kode Fasilitas"
        );
        checkExisting('#kodeFasilitas', "Kode Fasilitas");

        // Auto uppercase + minimal 2 huruf
        $('#kodeFasilitas').on('select2:select', function(e) {
            let val = $(this).val();
            if (!val) return;
            ...
            if (val.length < 2) {
                Swal.fire({
                    ...
                    text: 'Kode Fasilitas harus terdiri dari minimal 2 karakter.'
                }).then(() => {
                    $(this).val(null).trigger('change');
                });
                ...
                Swal.fire({
                    ...
                    text: 'Kode Fasilitas harus terdiri dari maksimal 7 karakter.'
                }).then(() => {
                    $(this).val(null).trigger('change');
                });
            }
        ...
        initSelect2(
            '#tipeFasilitas',
            "master-data/fasilitas/search/tipe",
            "Masukan Tipe Fasilitas"
        );

        // ============================
        // 3. SELECT2 Nama Fasilitas
        // ============================
        initSelect2(
            '#namaFasilitas',
            "master-data/fasilitas/search/nama",
            "Masukan Nama Fasilitas"
        );
        checkExisting('#namaFasilitas', "Nama Fasilitas");
    ...
    $('#modalEdit').on('shown.bs.modal', function() {
        ...
        $('#editKodeFasilitas').select2({
            ...
            width: "100%",
            ajax: {
                url: BASE_URL + "master-data/fasilitas/search/kode",
                dataType: 'json',
                delay: 0,
                data: params => ({
                    term: params.term
        ...
        $('#editTipeFasilitas').select2({
            ...
            width: "100%",
            ajax: {
                url: BASE_URL + "master-data/fasilitas/search/tipe",
                dataType: 'json',
                delay: 0,
                data: params => ({
                    term: params.term
        ...
        $('#editNamaFasilitas').select2({
            ...
            width: "100%",
            ajax: {
                url: BASE_URL + "master-data/fasilitas/search/nama",
                dataType: 'json',
                delay: 0,
                data: params => ({
                    term: params.term
    ...
    $('#formAdd').submit(function(e) {
        ...
        $.post(BASE_URL + "master-data/fasilitas/store", $(this).serialize(), function(res) {
    ...
    // EDIT SHOW DATA
    // ===================================
    $('#tblFasilitas').on('click', '.btn-edit', function() {
        const id = $(this).data('id');

        $.get(BASE_URL + "master-data/fasilitas/edit/" + id, function(res) {
    ...
    $('#formEdit').submit(function(e) {
        ...
        $.post(BASE_URL + "master-data/fasilitas/update/" + id, $(this).serialize(), function(res) {
    ...
    // DELETE DATA
    // ===================================
    $('#tblFasilitas').on('click', '.btn-delete', function() {
        const id = $(this).data('id');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.post(BASE_URL + "master-data/fasilitas/delete/" + id, function(res) {
    ...
    $('#modalAdd').on('hidden.bs.modal', function() {
        ...
        // Reset Select2
        $('#kodeFasilitas').val(null).trigger('change');
        $('#kodeFasilitas').empty();

        $('#tipeFasilitas').val(null).trigger('change');
        $('#tipeFasilitas').empty();

        $('#namaFasilitas').val(null).trigger('change');
        $('#namaFasilitas').empty();
