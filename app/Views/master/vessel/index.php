...
<?= $this->section('title') ?>Master Vessel<?= $this->endSection() ?>

<!-- ====================== HEADER ====================== -->
<?= $this->section('pageTitle') ?>
<div class="page-heading">
    <h3 class="heading-title">Master Data Vessel</h3>

    <p class="text-subtitle text-muted">
        Master Data Vessel digunakan untuk menyimpan informasi Vessel.
...
<div class="card">
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table id="tblVessel" class="table table-striped table-bordered w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Kode</th>
    ...
    <div class="modal-dialog modal-lg">
        <form id="formAdd" class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">Tambah Vessel</h5>
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="kode" id="KodeVessel" class="form-control select2" required></select>
                        <span id="kodeWarning" class="text-danger mt-1" style="display:none;">Kode sudah terdaftar!</span>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Nama Vessel</label>
                        <select name="nama_vessel" id="namaVessel" class="form-control select2" required></select>
                        <span id="namaWarning" class="text-danger mt-1" style="display:none;">Nama sudah terdaftar!</span>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Negara</label>
                        <select name="negara_vessel" id="negaraVessel" class="form-control select2" required></select>
                    </div>
        ...
        <form id="formEdit" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Kode</label>
                        <select name="kode" id="editKodeVessel" class="form-control select2" required></select>
                        <span id="editKodeWarning" class="text-danger mt-1" style="display:none;">Kode sudah terdaftar!</span>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Nama Vessel</label>
                        <select name="nama_vessel" id="editNamaVessel" class="form-control select2" required></select>
                        <span id="editNamaWarning" class="text-danger mt-1" style="display:none;">Nama sudah terdaftar!</span>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Negara</label>
                        <select name="negara_vessel" id="editNegaraVessel" class="form-control select2" required></select>
                    </div>
...
// INIT DATATABLE
// ===================================
let tbl = $('#tblVessel').DataTable({
    ajax: {
        url: BASE_URL + "master-data/vessel/list",
        dataSrc: 'data'
    },
    columns: [
        ...
        { 
            ...
            render: function(id){
                return `
                    <button class="btn btn-sm btn-warning btn-edit mb-2" data-id="${id}" title="Edit Vessel">
                        <i class="bi bi-pencil-square"></i>
                    </button>
                    <button class="btn btn-sm btn-danger btn-delete mb-2" data-id="${id}" title="Delete Vessel">
...
// ============================================
// 1. SELECT2 KODE Vessel (AUTO UPPERCASE)
// ============================================
initSelect2(
    '#KodeVessel',
    "master-data/vessel/search/kode",
    "Masukan Kode Vessel"
);
checkExisting('#KodeVessel', "Kode Vessel");

// Auto uppercase + minimal 4 huruf
$('#KodeVessel').on('select2:select', function (e) {
    let val = $(this).val();
    if (!val) return;
    ...
    if (val.length < 4) {
        Swal.fire({
            ...
            text: 'Kode Vessel harus terdiri dari minimal 4 karakter.'
        }).then(() => {
            $(this).val(null).trigger('change');
        });
        ...
        Swal.fire({
            ...
            text: 'Kode Vessel harus terdiri dari maksimal 6 karakter.'
        }).then(() => {
            $(this).val(null).trigger('change');
        });
    }
...
// ============================
// 2. SELECT2 NAMA Vessel
// ============================
initSelect2(
    '#namaVessel',
    "master-data/vessel/search/nama",
    "Masukan Nama Vessel"
);
checkExisting('#namaVessel', "Nama Vessel");
...
initSelect2(
    '#negaraVessel',
    "master-data/vessel/search/negara",
    "Masukan Negara"
);
...
// === KODE ===
$('#editKodeVessel').select2({
    dropdownParent: $('#modalEdit'),
    placeholder: "Masukan Kode Vessel",
    tags: true,
    width: "100%",
    ajax: {
        url: BASE_URL + "master-data/vessel/search/kode",
        dataType: 'json',
        delay: 0,
        data: params => ({ term: params.term }),
...
// === NAMA ===
$('#editNamaVessel').select2({
    dropdownParent: $('#modalEdit'),
    placeholder: "Masukan Nama Vessel",
    tags: true,
    width: "100%",
    ajax: {
        url: BASE_URL + "master-data/vessel/search/nama",
        dataType: 'json',
        delay: 0,
        data: params => ({ term: params.term }),
        processResults: data => ({ results: data })
...
// === NEGARA ===
$('#editNegaraVessel').select2({
    dropdownParent: $('#modalEdit'),
    placeholder: "Masukan Negara Vessel",
    tags: true,
    width: "100%",
    ajax: {
        url: BASE_URL + "master-data/vessel/search/negara",
        dataType: 'json',
        delay: 0,
        data: params => ({ term: params.term }),
        processResults: data => ({ results: data })
...
$('#formAdd').submit(function(e){
    ...
    $.post(BASE_URL + "master-data/vessel/store", $(this).serialize(), function(res){
...
// EDIT SHOW DATA
// ===================================
$('#tblVessel').on('click', '.btn-edit', function(){
    const id = $(this).data('id');

    $.get(BASE_URL + "master-data/vessel/edit/" + id, function(res){
        ...
        if(res.status === 'success'){
            ...
            $('#editKodeVessel').append(kodeOption).trigger('change');

            let namaOption = new Option(d.nama_vessel, d.nama_vessel, true, true);
            $('#editNamaVessel').append(namaOption).trigger('change');

            let negaraOption = new Option(d.negara_vessel, d.negara_vessel, true, true);
            $('#editNegaraVessel').append(negaraOption).trigger('change');
...
$('#formEdit').submit(function(e){
    ...
    $.post(BASE_URL + "master-data/vessel/update/" + id, $(this).serialize(), function(res){
...
// DELETE DATA
// ===================================
$('#tblVessel').on('click', '.btn-delete', function(){
    const id = $(this).data('id');
    ...
    Swal.fire({
        ...
        if(result.isConfirmed){
            ...
            $.post(BASE_URL + "master-data/vessel/delete/" + id, function(res){
...
$('#modalAdd').on('hidden.bs.modal', function () {
    ...
    // Reset Select2
    $('#KodeVessel').val(null).trigger('change');
    $('#KodeVessel').empty();

    $('#namaVessel').val(null).trigger('change');
    $('#namaVessel').empty();

    $('#negara_vessel').val(null).trigger('change');
    $('#negara_vessel').empty();
});
